...
const TransferList = ({
  ...
  const rows = transfers.map((row, index) => ({
    ...row,
    sno: index + 1,
    transaction_type_name: toTitleCase(row.transaction_type_name),
    amount: row.amount,
  ...
  const columns = [
    ...
    },
    {
      ...
      disableColumnMenu: true,
      renderCell: (params) => {
        const originalTransferItem = transfers.find(
        ...
        return (
          <div>
            <button
              ...
              onClick={() => onEdit(originalTransferItem)}
            >
              Edit
            </button>
            <button
              className="view-details-button"
              onClick={() => onViewDetails(originalTransferItem)}
            >
              Details
            </button>
            <button
              ...
              onClick={() => {
                setSelectedTransferId(originalTransferItem.id);
  ...
  return (
    <div>
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Delete Transfer"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            This action will permanently delete the selected transfer.
            <br />
        ...
        <DialogActions>
          <Button
            onClick={async () => {
              setIsDeleting(true);
              try {
                await onDelete(selectedTransferId);
              } catch (error) {
